...
describe('Time model test', () => {
  describe('constructor', () => {
    ...
    it('Will create a Time instance without any provided parameters', () => {
      ...
      expect(SUT.meridian).toEqual('');

      expect(utils.unitFixer).toHaveBeenCalledTimes(4);
      expect(utils.calculateDateOrdinal).toHaveBeenCalledTimes(1);
      expect(utils.hourFormatter).toHaveBeenCalledTimes(1);
      expect(utils.calculateMeridian).toHaveBeenCalledTimes(1);
    });
  ...
  describe('Instance Methods', () => {
    describe('incrementTime', () => {
      ...
      it('Will ignore non-number values', () => {
        ...
        SUT.incrementTime('hello');
        expect(SUT.timestamp).toEqual(fixedDate.getTime());

        SUT.incrementTime();
        expect(SUT.timestamp).toEqual(fixedDate.getTime());

        SUT.incrementTime(true);
        expect(SUT.timestamp).toEqual(fixedDate.getTime());
